...
class EvRepository(private val api: ServiceApi) : ResponseReceiver {
    ...
    suspend fun userInfoRepo(regStation: ResReg) = callApi { api.regStation(regStation) }

    suspend fun addRating(addRatingRequestBody: AddRatingRequestBody) = callApi { api.addRating(addRatingRequestBody) }
